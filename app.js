...
class DashboardItem {
    ...
    _createMarkup() {
        console.log(DashboardItem.periods[this.view])
...
document.addEventListener('DOMContentLoaded', () => {
    getDashboardData()
        .then(data => {
            ...
            selectors.forEach(sel => sel.addEventListener('click', () => {
                selectors.forEach(s => s.classList.remove('view-selector__item--active'))
                sel.classList.add('view-selector__item--active')

                const currentView = sel.innerText.trim().toLowerCase();
